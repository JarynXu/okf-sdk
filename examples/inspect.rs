...
fn main() -> ExitCode {
    ...
    println!("loaded {} documents from {}", bundle.len(), bundle.root().display());
    ...
    for issue in report.issues() {
        ...
        println!("{:?} {}{}: {}", issue.severity, issue.code, document, issue.message);
    }

    if let Some(query) = query {
        for hit in bundle.search(&SearchQuery::new(query).limit(10)) {
            println!("{}\t{}\t{}", hit.score, hit.document.id(), hit.document.title());
        }
    }
